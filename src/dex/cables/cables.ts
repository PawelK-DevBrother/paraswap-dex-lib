...
import { Adapters, CablesConfig } from './config';
import {
  ...
  CABLES_RATELIMIT_CACHE_VALUE,
  CABLES_RATE_LIMITED_TTL_S,
...
export class Cables extends SimpleExchange implements IDex<CablesData> {
  ...
  private cablesAuthToken: string;
  ...
  private tokensMap: CablesTokensResponse['tokens'] = {};
  ...
  constructor(
    ...
    readonly mainnetRFQAddress: string = CablesConfig['Dexalot'][network]
      .mainnetRFQAddress,
    ...
    this.cablesAuthToken = authToken;
  ...
  async getCachedTokens(): Promise<CablesTokensResponse['tokens'] | null> {
    ...
    if (cachedTokens) {
      return JSON.parse(cachedTokens) as CablesTokensResponse['tokens'];
    }
  ...
  ): Promise<null | ExchangePrices<CablesData>> {
    try {
      const normalizedSrcToken = this.normalizeToken(srcToken);
      const normalizedDestToken = this.normalizeToken(destToken);
      ...
      let clobSide = OrderbookSide.BID;
      if (pairData.isSrcBase) {
        orderbook = priceData.bids;
        clobSide = OrderbookSide.ASK;
      }
      ...
      const isInputQuote =
        (clobSide === OrderbookSide.ASK && side === SwapSide.SELL) ||
        (clobSide === OrderbookSide.BID && side === SwapSide.BUY);
  ...
  generateRFQError(errorStr: string, swapIdentifier: string) {
    ...
    throw new CablesRfqError(message);
  }
  ...
  getRestrictedPoolKey(poolIdentifier: string): string {
    return `${CABLES_RESTRICTED_CACHE_KEY}-${poolIdentifier}`;
  }

  async restrictPool(
    poolIdentifier: string,
    ttl: number = CABLES_RESTRICT_TTL_S,
  ): Promise<boolean> {
  ...
  async setBlacklist(
    txOrigin: Address,
    ttl: number = CABLES_BLACKLIST_CACHES_TTL_S,
  ): Promise<boolean> {
  ...
  async isRateLimited(txOrigin: Address): Promise<boolean> {
    ...
    return result === CABLES_RATELIMIT_CACHE_VALUE;
  }

  async setRateLimited(txOrigin: Address, ttl = CABLES_RATE_LIMITED_TTL_S) {
    await this.dexHelper.cache.setex(
      this.dexKey,
      this.network,
      this.getRateLimitedKey(txOrigin),
  ...
  getAPIReqParams(endpoint: string, method: Method): CablesAPIParameters {
    return {
      url: `${CABLES_API_URL}/${endpoint}`,
      headers: { 'x-apikey': this.cablesAuthToken },
      params: {
        chainid: this.network,
      },
      method: method,
